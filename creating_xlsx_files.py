...
class Save_module:
    ...
    def pegar_parte_superior(self,wb,nombre):
        ...
        parte_inferior = [['A','Toma de  muestra'],
                          ['A','1) Tome una muestra en un envase de 1/2 L (Debe estar limpio y seco)'],
                          ['A','2) COn un plumon, coloque el nombre del producto'],
                          ['A','3) Entregue la muestra al ICC junto con la orden de produccion correspondiente'],
                          ['C','1) Verifique que la malla este limpia y no tenga residuos de otro material.'],
                          ['C','2) Verifique que los envases no esten sucios y no tengan polvo'],
                          ['C','3) Al cerrar los envases verifique que esten bien cerrados'],
                          ['C','Envasado'],
                          ['D','Una vez aprobado su producto proceda a envasar de acuerdo a lo indicado en la orden de producción. Envase filtrando su material con una malla de 150 micras y envase con agitación'],
                          ]
        ...
        # PICTOGRAMAS
        img_a = openpyxl.drawing.image.Image('a.png')
        img_m = openpyxl.drawing.image.Image('m.png')
        img_c = openpyxl.drawing.image.Image('c.png')
        img_i = openpyxl.drawing.image.Image('i.png')
        img_t = openpyxl.drawing.image.Image('t.png')
                    ...
                    if i != len(self.lista_productos[str(op2)].keys())-1 and i != len(self.lista_productos[str(op2)].keys())-2:
                        ...
                        if peligro != '0':
                            for k,pictograma in enumerate(peligro):
                                if pictograma == 'a':
                                    print(f'{k}: fila{fila+k} y pictogrma {pictograma}')
                                    img_a.anchor = chr(66+k)+str(fila+2)
                                    self.ws.add_image(img_a)
                                elif pictograma == 'm':
                                    print(f'{k}: fila{fila+k} y pictogrma {pictograma}')
                                    img_m.anchor = chr(66+k)+str(fila+2)
                                    self.ws.add_image(img_m)
                                elif pictograma == 'c':
                                    print(f'{k}: fila{fila+k} y pictogrma {pictograma}')
                                    img_c.anchor = chr(66+k)+str(fila+2)
                                    self.ws.add_image(img_c)
                                elif pictograma == 'i':
                                    print(f'{k}: fila{fila+k} y pictogrma {pictograma}')
                                    img_i.anchor = chr(66+k)+str(fila+2)
                                    self.ws.add_image(img_i)
                                elif pictograma == 't':
                                    print(f'{k}: fila{fila+k} y pictogrma {pictograma}')
                                    img_t.anchor = chr(66+k)+str(fila+2)
                                    self.ws.add_image(img_t)
